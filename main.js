 ...
 *                 "http-errors", "express-ws", "consolidate", "ejs"]
 * local modules : ["./library.js", "./router/middleware.PostHandler.js"]
 * 
 * This is an http server using and extends from express app. The modules included
 * in this app is most downloaded and uses by many developper. We also use app.engine
 * to support some new extension. 
 * 
 ...
 * `next` function is not included, you should set it by your self by add {next} in render data. By
 * default, we set `next` in every router we declare below. It also included `{_locals, settings}` 
 ...
 * node module, but I set it to be engine for this app. It should exports a function with 3 
 ...
 * response it manually with `opts.res.send(html);`. This file also support routing. You access
